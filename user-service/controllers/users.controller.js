...
import { connectRabbitMQ, publishUserEvent } from '../lib/rabbitmq.js';
...
export const healthCheck = async (request,response) => {
    response.status(200).json({message:"user service is running"})
}
...
export const signup = async (request,response)=> {
    ...
    try{
        ...
        await publishUserEvent(rabbitMQChannel, 'UserCreated', {
            userId: user._id,
            name: user.name,
            email: user.email
        });
